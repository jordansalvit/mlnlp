...
def getCompleteDataSetByType(start,limit,cursor):
    cursor.execute(
        """select review_id, review, school, case when category = 'Chinese' then 'Chinese' ELSE 'American' END as categoryType from (
                (   SELECT  r.review_id, r.review, 'CMU' as school, category
                FROM Reviews r inner join businesses b on b.businessid = r.businessid
                inner join business_categories bc on b.businessid = bc.businessid
                inner join (
                    select r.review_id
                ...
                where b.schools = 'Carnegie Mellon University'
                and category in ('American (New)', 'Chinese')
                order by review_id
                limit %s, %s)
            union
            (   SELECT  r.review_id, r.review, 'Columbia' as school, category
                FROM Reviews r inner join businesses b on b.businessid = r.businessid
                inner join business_categories bc on b.businessid = bc.businessid
                inner join (
                    select r.review_id
                ...
                limit %s, %s)
            union
            (   SELECT  r.review_id, r.review, 'Princeton' as school, category
                FROM Reviews r inner join businesses b on b.businessid = r.businessid
                inner join business_categories bc on b.businessid = bc.businessid
                inner join (
                    select r.review_id
                ...
                limit %s, %s)
            union
            (   SELECT  r.review_id, r.review, 'Harvard' as school, category
                FROM Reviews r inner join businesses b on b.businessid = r.businessid
                inner join business_categories bc on b.businessid = bc.businessid
                inner join (
                    select r.review_id
                ...
                limit %s, %s)
            union
            (   SELECT  r.review_id, r.review, 'Urbana' as school, category
                FROM Reviews r inner join businesses b on b.businessid = r.businessid
                inner join business_categories bc on b.businessid = bc.businessid
                inner join (
                    select r.review_id
...
reviews = getCompleteDataSetByType(0,300,cursor)
...
reviews = getCompleteDataSetByType(0,225,cursor)
for review in reviews:
    try:
        train.write("%s " % review[0])
        train.write("%s " % review[2])
        train.write("%s " % review[3])
        listOfWords = re.findall(r'\w+', review[1])
        for word in listOfWords:
            train.write("%s " % word)
        train.write("\n")
...
reviews = getCompleteDataSetByType(225,75,cursor)
for review in reviews:
    try:
        test.write("%s " % review[0])
        test.write("%s " % review[2])
        test.write("%s " % review[3])
        listOfWords = re.findall(r'\w+', review[1])
        for word in listOfWords:
            test.write("%s " % word)
        test.write("\n")
